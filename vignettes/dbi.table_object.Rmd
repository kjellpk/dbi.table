...
`i` can be a call or a list of calls. When `i` is a list of calls, the calls are
combinded into a single call using the `AND` operator.

```{r ilist}
csql(Genre[.(GenreId > 5, GenreId < 10)])
```

Or, alternatively, using a single call.
...
```
 ...
 The calls are limited to
those that can be translated to SQL by `dbplyr::translate_sql_` 
...
The attached environment has a finalizer that will close the database
connection.